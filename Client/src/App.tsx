...
function App() {
  ...
  async function fetchTodo() {
    const response: Response = await fetch("/todo");
  ...
  return (
    <div className="flex h-svh w-full flex-col items-center justify-end gap-4 bg-gray-100 p-4 sm:justify-center lg:flex-col">
      ...
      <div className="flex h-full w-full flex-col-reverse sm:h-max sm:w-auto sm:gap-2 md:flex-row lg:h-max lg:flex-col">
        ...
        <div className="flex h-full flex-col gap-2 sm:py-0 md:h-max lg:h-full lg:flex-row">
          {todos?.length !== 0 ? (
            <div className="flex h-full w-full flex-col gap-2 overflow-y-auto rounded-lg border px-2 py-4 shadow sm:w-96 lg:h-96 lg:w-96 lg:gap-2">
              {todos}
            </div>
          ) : (
            <>
